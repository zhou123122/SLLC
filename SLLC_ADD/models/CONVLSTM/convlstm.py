...
class ConvLSTM_Audio(nn.Module):
    def __init__(self, input_dim, output_dim, conv_hidden, lstm_hidden, num_layers, output, activation, norm, dropout):
        super(ConvLSTM_Audio, self).__init__()
        self.conv = ConvBlock1d(in_channels=input_dim,      # 80
                                out_channels=conv_hidden,   # 256
                                kernel=3,
                                stride=1,
                                pad=1,
        ...
        self.lstm = nn.LSTM(input_size=conv_hidden,         # 256
                            hidden_size=lstm_hidden,        # 256
                            num_layers=num_layers,          # 4
                            batch_first=True,
                            bidirectional=True,
                            dropout=dropout)
        self.fc = FullyConnected(in_channels=lstm_hidden*2,   # 256
                                 out_channels=output_dim,   # 2
                                 activation=activation,     
                                 normalisation=norm)    
        self.evaluator = MLP_block(output_dim, output)
    ...
    def forward(self, net_input):
        ...
        x = x[:, -1, :].reshape(batch, -1)
        probs = self.fc(x)         # output shape: (batch, output_dim)
        probs = self.evaluator(probs)
